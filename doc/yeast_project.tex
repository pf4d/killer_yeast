...
\end{align}
The quantitative measurement for fitness is a unitless measurement of optical density at steady state ($N$) at a given flow rate ($F$) in volumes/hr.  In order to find the steady states, we have to find the intersections of the null-clines at equilibrium points $(\bar{N}, \bar{C})$, i.e.\ $d\bar{N}/dt = 0$ and $d\bar{C}/dt = 0$:
\begin{align*}
...
\begin{figure}[H]
  ...
  \caption{\footnotesize The $\dot{C} = 0$ nullcline (red) intersecting with the $\dot{N} = 0$ nullcline (black).  The trivial steady-state $(0,0)$ and non-trivial steady-state $(\bar{N}, \bar{C})$ are shown as red dots.}
\end{figure}
\vfill
\columnbreak
...
After we obtain estimates for $\alpha$ and $K$ (and hence $\bar{C}$), we can model a ``what if'' scenario whereby we place both species of yeast, sensitive and killer, into one chemostat.
